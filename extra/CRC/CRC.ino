...
// define DataArray for Calculation (Data + CRC)                        {--------------------------DATA----------------------,-CRC-}
uint8_t                  CRC_DATA_BUFFER_CALCULATION[CRC_DATA_LENGTH] = {0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, 0x00};

// define DataArray for Integrity Check (Data + CRC)                   {--------------------------DATA----------------------,-CRC-}
uint8_t                  CRC_DATA_BUFFER_Validation[CRC_DATA_LENGTH] = {0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, 0xF4};
...
void setup() {
  ...
  uint8_t CRC_OUT = calculateCRC8(CRC_DATA_BUFFER_CALCULATION, CRC_DATA_LENGTH, CRC_POLYNOMIAL);

  // update CRC-Byte in Data array
  CRC_DATA_BUFFER_CALCULATION[CRC_DATA_LENGTH-1] = CRC_OUT;

  // return result via Serial monitor
  Serial.print("CRC-8: 0x");
  Serial.println(CRC_DATA_BUFFER_CALCULATION[CRC_DATA_LENGTH-1], HEX);

  // check integrity with CRC methode
  bool Integrity = checkDataIntegrity(CRC_DATA_BUFFER_Validation, CRC_DATA_LENGTH, CRC_POLYNOMIAL);
